...
## Research and Web Search (Read-Only)

When external information, online API documentation, library specs, GitHub issues, or public error solutions are needed (in either Plan Mode or Normal Mode), automatically dispatch `tiny` to perform web searches and documentation retrieval, keeping the main context clean. `tiny` is strictly read-only and never modifies repository files. Do not dispatch `backend` or `frontend` for research tasks.

## Git Management (Commits & Push)

When the user asks to stage, commit, generate git logs/history, or push changes, dispatch `git`. `git` inspects git diffs, creates concise conventional commits, and handles push operations upon user confirmation.
